...
export const InterfazCriticaDemo = () => {
  ...
  useEffect(() => {
    const mark = () => {
      actividadRef.current.lastActivity = performance.now();
    };
    ...
    const tick = setInterval(() => {
      ...
      const dt = now - actividadRef.current.lastTick;
      actividadRef.current.lastTick = now;

      const idleMs = now - actividadRef.current.lastActivity;

      if (idleMs < 4500) {
        actividadRef.current.activeMs += dt;
      }
      ...
      if (
        idleMs > 16000 &&
        now - actividadRef.current.lastIdleNoticeAt > 60000
      ) {
        actividadRef.current.lastIdleNoticeAt = now;
        setNotificacion({
          tipo: 'pregunta',
          mensaje: '¿Sigues aquí o dejaste la pestaña abierta?',
        });
        return;
      }

      if (
        actividadRef.current.activeMs > 120000 &&
        now - actividadRef.current.lastBreakNoticeAt > 120000
      ) {
        actividadRef.current.lastBreakNoticeAt = now;
        const mins = Math.max(1, Math.round(actividadRef.current.activeMs / 60000));
        setNotificacion({
          tipo: 'insight',
          mensaje: `Llevas ${mins} min en fricción. ¿Pausa de 30s?`,
          onAction: () => {
            actividadRef.current.lastActivity = performance.now();
          },
        });
    ...
    <div style={{ 
      ...
      <div style={{ maxWidth: '500px' }}>
        <section style={{ marginBottom: '3rem' }}>
          ...
          <BotonCritico
            ...
            onConfirm={() => {
              ...
              setNotificacion({
                tipo: 'info',
                mensaje: 'La interfaz no te detuvo: solo te hizo pensar.',
              });
            }}
            onCancel={() => {
              setCancelCount((c) => {
                ...
                if (next >= 2) {
                  setNotificacion({
                    tipo: 'insight',
                    mensaje: 'Cancelaste dos veces. Esa duda es información.',
                  });
                }
                return next;
              });
        ...
        <section style={{ marginBottom: '3rem' }}>
          ...
          <InputReflexivo
            ...
            onSubmit={(valor, analisis) => {
              if (analisis?.claridad === 'confuso') {
                setNotificacion({
                  tipo: 'pregunta',
                  mensaje: '¿Quieres ser entendido o solo ser rápido?',
                });
                return;
              }

              if (analisis?.tono === 'tenso') {
                setNotificacion({
                  tipo: 'advertencia',
                  mensaje: 'Tu tono suena tenso. ¿Quieres suavizarlo antes de enviar?',
                });
                return;
              }
              ...
              setNotificacion({
                tipo: 'info',
                mensaje: 'Enviado. El sistema respondió, pero no te premió con ruido.',
              });
              console.log('Enviado:', valor, analisis);
            }}
          />
        </section>
      </div>
      ...
      {notificacion && (
        <NotificacionCritica
          ...
          persistente={false}
          onDismiss={() => setNotificacion(null)}
      ...
      <div style={{
        ...
        decisiones: {confirmCount} confirmaciones · {cancelCount} cancelaciones
      </div>
    </div>
  );
};
