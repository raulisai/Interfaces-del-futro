...
export const ComponentesAdhesivosDemo = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Activar isla aleatoriamente
      if (Math.random() > 0.7) {
        setIslaActiva(true);
        setTimeout(() => setIslaActiva(false), 5000);
      }
      
      // Cambiar estado del borde
      const estados = ['dormido', 'atento', 'activo', 'urgente'];
      setEstadoBorde(estados[Math.floor(Math.random() * estados.length)]);
    }, 8000);
